  ...
  switch (state) {
    case TURNING:
      if (currentMillis - turnStartMillis >= turnInterval) {
        ...
        base_angle = MOTORS.get_default_angle((direction != 0) ? abs(direction - 1) : 1); // making base angle the right angle for the servo (based on turn direction this changes to left or right angle of the servo)
      }
      break;
